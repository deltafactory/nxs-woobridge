...
{	
	...
	if (false)
	{
		if (nxs_shop_isloggedon())
		{
			$email = nxs_shop_getuseremail();
			$name = nxs_shop_getusername();
			$credits = nxs_shop_getremainingcredits();
			
			echo "Welcome {$name} you have {$credits} credits!";
			echo "<a href='#' onclick='nxs_js_shop_logout(); return false;'>[Logout link]</a>";
			?>
			<script type='text/javascript'>
				function nxs_js_shop_logout()
				{
					nxs_js_setcookie("nxs_shop_usertoken", "");
					nxs_js_alert("Cookie wiped");
					// todo: refresh						
				}
			</script>
			<?php
		...
		{
			echo "<a href='#' onclick='nxs_js_shop_login(); return false;'>[Login link]</a>";
			?>
			<script type='text/javascript'>
				function nxs_js_shop_login()
				{
					nxs_js_setcookie("nxs_shop_usertoken", "klkjhg342o87y3f2iugh");
					nxs_js_alert("Cookie set");
					// todo: refresh
				}
			</script>
			<?php
	...
	if (true)
	{
		...
		else 
		{
			...
			if ($found)
			{
				...
				if ($nxs_global_current_containerpostid_being_rendered == $cart_post_id)
				{
					// this is the cart page itself, don't show anything
				}
				else
				{
